...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        img: cardNode.getElementsByTagName('img'), // здесь картинка внутри .product
        productName: cardNode.querySelector('.productName'), // здесь .productName, который внутри .product
        button: cardNode.getElementsByTagName('button'), // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    
    let textButton = card.button[0].innerText;
...
function hideMoreText(card) {
    ...
    card.img[0].style.display = 'block';
    ...
    card.button[0].innerHTML = 'Подробнее';
}
